...
class ScreenTestCase(unittest.TestCase):
    def test(self):
        ...
        screen.rect(3, 2)
        self.assertEqual((ON,) * 3 + (OFF,) * 4 + (ON,) * 3 + (OFF,) * 11, screen.state)
        screen.rotate_column_x(1, 1)
        self.assertEqual((ON, OFF, ON) + (OFF,) * 4 + (ON,) * 3 + (OFF,) * 5 + (ON,) + (OFF,) * 5,
                         screen.state)
        screen.rotate_row_y(0, 4)
        self.assertEqual((OFF,) * 4 + (ON, OFF) + (ON,) * 4 + (OFF,) * 5 + (ON,) + (OFF,) * 5,
                         screen.state)
        screen.rotate_column_x(1, 1)
        self.assertEqual((OFF, ON, OFF, OFF, ON, OFF, ON, ON, OFF, ON) + (OFF,) * 5 + (ON,) + \
                         (OFF,) * 5, screen.state)
